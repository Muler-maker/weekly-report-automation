...
# ---- Keyword variants (prefilter before GPT to boost recall + reduce cost) ----
KW = [
    r"\blu[\s\-]?177\b", r"\b177\s*lu\b", r"\b177lu\b", r"\b177-lu\b",
    r"\blutetium\b", r"\blutetio\b", r"\blu-?177\b",
    r"\btb[\s\-]?161\b", r"\b161\s*tb\b", r"\b161tb\b", r"\b161-tb\b",
    r"\bterbium\b"
]
...
def summarize_text(text: str):
    """Ask GPT to confirm Lu-177 / Tb-161 tender relevance."""
...
for url in rss_sources:
    try:
        ...
        for e in entries[:50]:
            ...
            blob = f"{title}\n{summary}"
            if not KW_RE.search(blob):
                continue  # skip fast if no isotope hint
...
# ---- Process hospital procurement pages (simple HTML scan) ----
