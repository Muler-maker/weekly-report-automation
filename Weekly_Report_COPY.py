...
def send_email(subject, body, to_emails, attachment_path):
    if not os.path.exists(attachment_path):
        print(f"❌ Attachment file not found: {attachment_path}"); return
...
# --- MODIFICATION START ---
# Create a clean version of the report text for the executive summary
with open(summary_path, "r", encoding="utf-8") as f:
    raw_report_text_for_exec_summary = f.read()

# Create a version with historical context for the detailed insights
...
# --- MODIFICATION: Use the clean report text for the summary ---
...
# --- ADDED: Print summary for debugging ---
...
with PdfPages(latest_pdf) as pdf:
    ...
    if not any([stopped, decreased, increased, inactive_recent_4]):
        ...
        fig_fallback.text(0.5, 0.5, "No trend data available.", ha="center", va="center", fontsize=18)
        pdf.savefig(fig_fallback); plt.close(fig_fallback)
    else:
        # ALL subsequent PDF generation logic is restored
        ...
        summary_df = pd.DataFrame([["Stopped", len(stopped), "–"],["Decreased", sum(len(v) for v in decreased_by_am.values()), f"{total_decreased:+.1f}"],["Increased", sum(len(v) for v in increased_by_am.values()), f"{total_increased:+.1f}"],["Inactive", len(inactive_recent_4), "–"]], columns=["Category", "# Customers", "Total Δ (mCi)"])
        ...
        # ... (And so on for all other tables and pages)
